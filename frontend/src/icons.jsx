...
export function Icon({ name, className = 'ic' }) {
  const common = { className, viewBox: '0 0 24 24', ...P }
  switch (name) {
    case 'dashboard':
    ...
    case 'models':
      return <svg {...common}><path d="M4 19V5"/><path d="M4 18h16"/><path d="m7 15 3-4 3 2 4-6"/><circle cx="7" cy="15" r="1"/><circle cx="10" cy="11" r="1"/><circle cx="13" cy="13" r="1"/><circle cx="17" cy="7" r="1"/></svg>
    case 'bell':
...
export function initials(n) {
  return (n || '?').replace(/\(.*\)/, '').replace(/<.*>/, '').trim().split(/\s+/).slice(0, 2).map(w => w[0]).join('').toUpperCase()
}
